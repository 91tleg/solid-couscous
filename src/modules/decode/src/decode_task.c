...
static QueueHandle_t decoded_data_queue;
...
static void decode_task(void *parameters)
{
    ...
    for (;;)
    {
        if (xQueueReceive(uart_data_queue, &d, portMAX_DELAY))
        {
            switch (d.state)
            {
            case STATE_ROMID:
            ...
            case STATE_INPUT_SWITCHES:
                sm_data.params.in_sw = decode_input_switches(d.value);
                break;

            case STATE_INOUT_SWITCHES:
                sm_data.params.io_sw = decode_io_switches(d.value);
                break;

            case STATE_ACTIVE_CODE_ONE:
            case STATE_STORED_CODE_ONE:
                sm_data.params.tc_one = decode_trouble_code_one(d.value);
                break;

            case STATE_ACTIVE_CODE_TWO:
            case STATE_STORED_CODE_TWO:
                sm_data.params.tc_two = decode_trouble_code_two(d.value);
                break;

            case STATE_ACTIVE_CODE_THREE:
            case STATE_STORED_CODE_THREE:
                sm_data.params.tc_three = decode_trouble_code_three(d.value);
                break;
            }

            xQueueSend(decoded_data_queue, &sm_data, 0);
        }
    }
}
...
void decode_task_init(QueueHandle_t uart_data_q, QueueHandle_t decoded_data_q)
{
    ...
    decoded_data_queue = decoded_data_q;
